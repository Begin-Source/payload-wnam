...
function categorySlugsFromArticle(a: Article): Set<string> {
...
function countForChipSlug(articles: Article[], chipSlug: string): number {
  return articles.reduce((n, a) => (categorySlugsFromArticle(a).has(chipSlug) ? n + 1 : n), 0)
}
...
export function AmzReviewsBrowseClient({
  ...
  const replaceQuery = useCallback(
    (category: string | null, search: string) => {
      ...
      const qs = p.toString()
      const path = pathname ?? amzNavHref(locale, '/reviews')
  ...
  const filtered = useMemo(() => {
    ...
    let out = list
    if (activeSlug) {
      out = out.filter((a) => categorySlugsFromArticle(a).has(activeSlug))
    }
    if (searchForFilter) {
  ...
  const sidebarInner = (
    <nav className="space-y-1">
      ...
      >
        <span>
          {allReviewsLabel} ({articles.length})
        </span>
      </button>
      {chips.map((c) => {
        const cnt = countForChipSlug(articles, c.slug)
        return (
          <button
            key={c.slug}
            type="button"
        ...
        {filtered.length === 0 ? (
          ...
          <ul className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
            {filtered.map((article) => (
              <li key={article.id}>
                <AmzReviewListingCard article={article} locale={locale} />
              </li>
            ))}
          </ul>
        )}
