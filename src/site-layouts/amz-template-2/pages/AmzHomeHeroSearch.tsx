...
export function AmzHomeHeroSearch({
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto mt-10 w-full max-w-2xl">
      <div className="flex min-h-[3.25rem] items-stretch overflow-hidden rounded-full border border-border bg-card pl-4 shadow-lg ring-1 ring-border/40 sm:pl-5">
        <div className="flex min-w-0 flex-1 items-center gap-3 py-2 pr-2">
          ...
          <input
            ...
            onChange={(e) => setQ(e.target.value)}
            autoComplete="off"
            className="min-w-0 flex-1 border-0 bg-transparent py-2 text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-0 md:text-[17px]"
          />
        </div>
        <Button
          type="submit"
          className="h-auto shrink-0 rounded-none rounded-r-full bg-accent px-6 text-sm font-semibold text-accent-foreground hover:bg-accent/90 sm:px-8 sm:text-base"
        >
          Search
        </Button>
      </div>
