...
export type RunCategoryCoverGenerateOk = {
  ok: true
  categoryId: number
  mediaId: number
  mode: 'replace' | 'create_and_attach'
  mimeType: string
}

export type RunCategoryCoverGenerateErr = {
  ok: false
  categoryId?: number
  error: string
  message?: string
...
export type RunCategoryCoverGenerateResult = RunCategoryCoverGenerateOk | RunCategoryCoverGenerateErr

/**
 * Core Together category cover generation (shared by pipeline route and admin sync).
 * Uses `overrideAccess: true` for internal reads/writes; callers must enforce auth/tenant gates.
 */
export async function runCategoryCoverGenerate(
  payload: Payload,
  categoryId: number,
  options?: { expectedSiteId?: number | null; prompt?: string | null },
): Promise<RunCategoryCoverGenerateResult> {
  const bodySite =
    typeof options?.expectedSiteId === 'number' && Number.isFinite(options.expectedSiteId)
      ? options.expectedSiteId
      : null
  const overridePrompt = typeof options?.prompt === 'string' ? options.prompt : null
  ...
  if (!cat) {
    return { ok: false, categoryId, error: 'category_not_found', httpStatus: 404 }
  }

  const cid = cat as Category
  const catSiteNum = siteIdFromCategory(cid)
  if (catSiteNum == null) {
    return { ok: false, categoryId, error: 'category has no site', httpStatus: 422 }
  }
  if (bodySite != null && bodySite !== catSiteNum) {
    return { ok: false, categoryId, error: 'siteId mismatch vs category.site', httpStatus: 403 }
  }
  ...
  if (!siteForTenant) {
    return { ok: false, categoryId, error: 'site_not_found', httpStatus: 404 }
  }
  ...
  if (!promptText.trim()) {
    return { ok: false, categoryId, error: 'empty prompt', httpStatus: 400 }
  }
  ...
  try {
    ...
    if (existingMediaId != null) {
      ...
      if (!mdoc) {
        return { ok: false, categoryId, error: 'cover media missing', httpStatus: 404 }
      }
      ...
      if (mediaSid == null) {
        return {
          ok: false,
          categoryId,
          error: 'cover media missing site on row',
          httpStatus: 422,
        }
      }
      if (mediaSid !== catSiteNum) {
        return {
          ok: false,
          categoryId,
          error: 'cover media site mismatches category site',
          httpStatus: 403,
        }
      }
