...
async function hashToken(token: string): Promise<string> {
  if (!/^[0-9a-f]{64}$/.test(token)) throw new Error('Invalid site login token')
...
export class SiteLoginBroker {
  ...
  private async authorize(siteId: string, userId: string, sessionId: string) {
    ...
    if (!site || site.migrationState !== 'active' || !identity || identity.userId !== userId ||
      identity.sessionId !== sessionId || !Number.isFinite(identity.expiresAt) || identity.expiresAt <= this.now()) throw new Error('Site access denied')
    ...
    if (!grant || !['viewer','editor','publisher','manager'].includes(grant.role)) throw new Error('Site access denied')
  ...
  async redeemTicket(ticket: string, trustedSiteId: string, adminHost: string) {
    ...
    if (!row || row.siteId !== trustedSiteId || row.adminHost !== adminHost || row.expiresAt <= this.now()) throw new Error('Ticket invalid or expired')
    const live = await this.authorize(row.siteId, row.userId, row.centralSessionId)
    if (live.site.routingVersion !== row.routingVersion) throw new Error('Ticket routing changed')
    ...
    if (results[1].meta.changes !== 1) throw new Error('Ticket already redeemed or access revoked')
  ...
  async authenticate(session: string, trustedSiteId: string, adminHost: string): Promise<SitePrincipal> {
    ...
    if (!row || row.siteId !== trustedSiteId || row.adminHost !== adminHost || row.expiresAt <= this.now()) throw new Error('Site session invalid or expired')
    const live = await this.authorize(row.siteId, row.userId, row.centralSessionId)
    if (live.site.routingVersion !== row.routingVersion) throw new Error('Site session routing changed')
