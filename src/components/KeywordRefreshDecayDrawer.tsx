...
function clampDecayThreshold(raw: string): number {
  const n = Number(raw)
...
export const KeywordRefreshDecayDrawer = forwardRef<KeywordDrawerRef>(function KeywordRefreshDecayDrawer(
  ...
  const [preview, setPreview] = useState<{
    pickedTerms: string[]
    pickedRefreshMeta: Array<{ keywordId: number; articleId: number; decayScore: number; decayReason: string }>
    wouldEnqueue: number
  } | null>(null)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return
        const p = data.preset
        if (p?.batchMode === 'refresh_decay' && typeof p.decayThreshold === 'number' && Number.isFinite(p.decayThreshold)) {
          setDecayThreshold(String(p.decayThreshold))
        }
  ...
  const fetchPreview = useCallback(async () => {
    ...
    const th = clampDecayThreshold(decayThreshold)
    ...
    try {
      ...
      setPreview({
        ...
        pickedRefreshMeta: data.pickedRefreshMeta ?? [],
        wouldEnqueue: typeof data.enqueued === 'number' ? data.enqueued : 0,
      })
    } finally {
      setPreviewLoading(false)
    }
  }, [selectedSiteId, decayThreshold, batchLimitInput])
  ...
  const previewCandidatesInBackground = (): void => {
    ...
    void (async () => {
      try {
        ...
        const rmeta = Array.isArray(data.pickedRefreshMeta) ? data.pickedRefreshMeta : []
        const notices = (Array.isArray(data.errorsSample) ? data.errorsSample : []).slice(0, 6)
        const detailLines =
          rmeta.length > 0
            ? rmeta.slice(0, 8).map(
                (m) => `kw ${m.keywordId} / 文 ${m.articleId} — ${m.decayReason}（${m.decayScore.toFixed(2)}）`,
              )
            : undefined
        completeKeywordBatchModePreviewJob({
          jobId,
  ...
  const titleId = 'keyword-refresh-decay-title'
          ...
          <div style={{ ...backdropStyle, pointerEvents: 'none' }}>
            ...
            >
              <h2 id={titleId} style={{ margin: '0 0 0.75rem', fontSize: '1.125rem', fontWeight: 600 }}>
                衰减刷新 → content_refresh
              </h2>
              <div style={warnBox}>
                <strong>注意</strong>：依赖已发布文章上的 <code>primaryKeyword</code> 与 <code>rank_track</code> 写的{' '}
                <code>rankings.serpPosition</code>。<code>/api/pipeline/content-refresh</code> 当前仍为占位实现，任务会标记
               完成但不会自动改正文；后续再接真实刷新逻辑。
              </div>
              <p style={{ margin: '0 0 1rem', fontSize: '0.8125rem', opacity: 0.85 }}>
                无足够 rank 快照时仅按「未刷新时长」给低分；若预览为空可降低阈值或先跑 rank_track。「预览候选」关窗后顶栏 dry-run；此处点「刷新预览」看条目。
              </p>
              {error ? (
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div ref={siteComboboxRef} style={{ position: 'relative' }}>
                  ...
                  >
                    {selectedSiteLabel || '选择…'} <span style={{ opacity: 0.6 }}>▾</span>
                  </button>
                  {siteMenuOpen ? (
                    <div
                      style={{
                    ...
                    >
                      <input style={inputStyle} value={siteQuery} onChange={(e) => setSiteQuery(e.target.value)} />
                      {sitesLoading ? <div style={{ padding: 8 }}>…</div> : null}
                      {sites.map((s) => (
                        <button
                          key={s.id}
                          type="button"
              ...
              <div style={{ marginBottom: '1rem' }}>
                <label style={fieldLabel}>衰减分阈值（{decayThreshold}）</label>
                <input
                  type="range"
                  min={0.1}
                  max={0.95}
                  step={0.05}
                  value={Number(decayThreshold)}
              ...
              {preview && selectedSiteId != null ? (
                <div style={{ marginBottom: '1rem', padding: '0.65rem', border: '1px solid var(--theme-elevation-150)', borderRadius: 6, fontSize: '0.75rem' }}>
                  <strong>预览</strong> {previewLoading ? '…' : null}
                  <p style={{ margin: '0.35rem 0 0' }}>将入队 {preview.wouldEnqueue} 条 content_refresh</p>
                  <ul style={{ margin: '0.25rem 0 0', paddingLeft: '1rem', maxHeight: 160, overflow: 'auto' }}>
                    {preview.pickedRefreshMeta.slice(0, 15).map((m) => (
                      <li key={`${m.keywordId}-${m.articleId}`}>
                        kw {m.keywordId} / 文 {m.articleId} — {m.decayReason}{' '}
                        <span style={{ opacity: 0.8 }}>({m.decayScore.toFixed(2)})</span>
                      </li>
                    ))}
                  </ul>
                </div>
