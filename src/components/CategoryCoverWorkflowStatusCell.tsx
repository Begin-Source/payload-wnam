...
/** List cell for `categoryCoverWorkflowStatus` — Together category cover (admin sync path). */
export function CategoryCoverWorkflowStatusCell(props: Props): React.ReactElement {
  const row = props.rowData as Record<string, unknown> | undefined
  const fromCell = props.cellData
  const fromRow = row?.categoryCoverWorkflowStatus
