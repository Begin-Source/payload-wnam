...
const PATH = '/api/pipeline/site-hero-banner-generate'
...
export async function POST(request: Request): Promise<Response> {
    ...
    return Response.json(
      {
        ...
        message: '首页横幅仅适用于 amz-template-1 / amz-template-2。',
      },
      { status: 422 },
    )
  }
  ...
  try {
    ...
    const base = slug
      ...
      .replace(/^-|-$/g, '')
      .slice(0, 60)
    const name = `${base || `site-${siteId}`}-hero-banner-${Date.now()}.${ext}`
    ...
    if (existingMediaId != null) {
      ...
      if (!mdoc) {
        return Response.json({ ok: false, error: 'hero banner media missing' }, { status: 404 })
      }
      ...
      if (mediaSid == null || mediaSid !== siteId) {
        return Response.json(
          {
            ok: false,
            error: 'banner media site mismatch',
            message: '现有横幅媒体不属于该站点，请先在后台解除关联或使用正确站点。',
          },
          { status: 403 },
        )
      }
      ...
      await payload.update({
        ...
        data: {
          ...
          aiImageGenError: '',
          aiImageGenAt: nowIso,
          aiImagePromptSource: 'homepage_hero_auto',
          alt: `${siteName} homepage hero backdrop`,
      ...
      await incrementSiteQuotaUsage(payload, siteId, {
        imagesUsd: resolveTogetherImageChargeUsd({ raw, kind: 'homepage_hero_auto' }).usd,
      })
    ...
    await payload.logger.error({ err: stored, siteId }, 'site_hero_banner_generate_failed')
    return Response.json(
      {
        ok: false,
        error: 'site_hero_banner_generate_failed',
        message: msg,
        siteId,
        detail: stored,
      },
