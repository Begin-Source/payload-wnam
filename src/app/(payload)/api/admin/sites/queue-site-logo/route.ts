...
async function pendingSiteLogoJob(payload: PayloadInstance, siteIdNum: number): Promise<boolean> {
  const r = await payload.find({
    collection: 'workflow-jobs',
    where: {
      and: [{ jobType: { equals: 'site_logo_generate' } }, { status: { in: ['pending', 'running'] } }],
    },
    limit: 150,
    depth: 0,
    overrideAccess: true,
...
/** POST — enqueue Together site logos / favicons (workflow `site_logo_generate`). */
export async function POST(request: Request): Promise<Response> {
  ...
  for (const sid of siteIds) {
    try {
      ...
      if (await pendingSiteLogoJob(payload, sid)) {
      ...
      await payload.create({
        collection: 'workflow-jobs',
        data: {
          label: `Together 站点 Logo · site #${sid}`.slice(0, 120),
          jobType: 'site_logo_generate',
          status: 'pending',
          site: sid,
