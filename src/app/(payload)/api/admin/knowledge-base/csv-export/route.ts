...
const CSV_HEADER =
  'id,slug,title,status,notes,site_id,body_json,entry_type,skill_id,subject,summary,payload_json,severity,expires_at,artifact_class'
...
export async function GET(request: Request): Promise<Response> {
  ...
  while (true) {
    ...
    for (const doc of result.docs) {
      ...
      const payloadJson = doc.payload == null ? '' : JSON.stringify(doc.payload)
      const row = [
        String(doc.id),
        escapeCsvCell(doc.slug == null ? '' : String(doc.slug)),
        escapeCsvCell(doc.title ?? ''),
        escapeCsvCell(doc.status ?? ''),
        escapeCsvCell(doc.notes == null ? '' : String(doc.notes)),
        escapeCsvCell(docSiteId(doc)),
        escapeCsvCell(body),
        escapeCsvCell(doc.entryType == null ? '' : String(doc.entryType)),
        escapeCsvCell(doc.skillId == null ? '' : String(doc.skillId)),
        escapeCsvCell(doc.subject == null ? '' : String(doc.subject)),
        escapeCsvCell(doc.summary == null ? '' : String(doc.summary)),
        escapeCsvCell(payloadJson),
        escapeCsvCell(doc.severity == null ? '' : String(doc.severity)),
        escapeCsvCell(doc.expiresAt == null ? '' : String(doc.expiresAt)),
        escapeCsvCell(doc.artifactClass == null ? '' : String(doc.artifactClass)),
      ].join(',')
      lines.push(row)
    }
  ...
  return new Response(csv, {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="knowledge-base-${stamp}.csv"`,
    },
  })
}
