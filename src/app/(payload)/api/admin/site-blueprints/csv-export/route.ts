...
export async function GET(request: Request): Promise<Response> {
  ...
  while (true) {
    ...
    for (const doc of result.docs) {
      const cfg =
        doc.templateConfig != null && typeof doc.templateConfig === 'object'
          ? JSON.stringify(doc.templateConfig)
          : doc.templateConfig != null
            ? String(doc.templateConfig)
            : ''
      const row = [
        String(doc.id),
  ...
  return new Response(csv, {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="site-blueprints-${stamp}.csv"`,
    },
  })
}
