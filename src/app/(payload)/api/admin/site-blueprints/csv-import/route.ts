...
export async function POST(request: Request): Promise<Response> {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const templateJson = col('templateconfig_json', row).trim()
    let templateConfig: unknown = undefined
    if (templateJson) {
      try {
        templateConfig = JSON.parse(templateJson) as unknown
      } catch {
        errors.push({ row: lineNum, message: 'invalid templateConfig_json' })
        continue
      }
    }
    ...
    try {
      if (idStr) {
        ...
        if (templateConfig !== undefined) data.templateConfig = templateConfig
        if (prevSite == null) {
          data.site = siteId
