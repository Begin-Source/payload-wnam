...
const LEVEL_VALUES = new Set(['intro', 'standard', 'advanced'])
...
export async function POST(request: Request): Promise<Response> {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const levelRaw = col('level', row).trim()
    const statusRaw = col('status', row).trim()
    const summary = col('summary', row)
    const searchKeywords = col('search_keywords', row)
    const bodyJson = col('body_json', row).trim()
    const sortOrderRaw = col('sort_order', row).trim()
    ...
    if (levelRaw && !LEVEL_VALUES.has(levelRaw)) {
      errors.push({ row: lineNum, message: 'invalid level' })
      continue
    }
    ...
    if (sortOrderRaw !== '') {
      ...
      if (!Number.isFinite(n)) {
        errors.push({ row: lineNum, message: 'invalid sort_order' })
        continue
      }
      sortOrder = n
    }
    ...
    try {
      if (idStr) {
        ...
        if (levelRaw) data.level = levelRaw
        ...
        const data: Record<string, unknown> = {
          title,
          level: levelRaw ? levelRaw : 'standard',
          status: statusRaw ? statusRaw : 'draft',
        }
        if (slug) data.slug = slug
        if (summary) data.summary = summary
        if (searchKeywords) data.searchKeywords = searchKeywords
        if (body !== undefined) data.body = body
        if (sortOrder !== undefined) data.sortOrder = sortOrder
