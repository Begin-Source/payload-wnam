...
完整角色测试生成单独的 schema-only 文件；远程初始化不读取带账号/数据的 fixture 文件。`p1-schema.ts` 在空库中建立固定操作 ID、schema 摘要与完成状态。相同操作可续跑并保留数据；未知对象、不同摘要、已完成库出现缺失或变更均拒绝，不自动删表或改表。这是专用 P1 新库初始化，不代表通用 fleet-migrate 已完成。
