...
这是明确选择的一次合成申请运维执行，自动 Deploy Hook、构建投递与状态对账尚未启用。`05cf0c4` 移除 `admission` 选择后，[五站普通发布及 405 表核验](site-per-d1-p1-five-site-release.md)已于 04:35Z 实际通过，C/D/E 原建站回执保持。此处的建站绑定证明与后续普通发布验收分别归档。
