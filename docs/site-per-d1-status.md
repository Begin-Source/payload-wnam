...
- 已创建 [两个远程测试 D1](site-per-d1-p0-resources.json)，关闭读副本。A 库已执行部分历史迁移，B 库仍为空；尚未绑定已部署测试 Worker。云端 fixture 使用自己的本地隔离 D1，不能混淆这两组资源。
- 实现已推送到 `feat/site-per-d1`。提交 `c7cf614` 的原型构建成功。`f39f791` 构建及 14 项浏览器检查通过，随后新库迁移因 CLI 文件排序先 ALTER 后 CREATE 失败，未部署 Worker；已改为显式使用仓库 migrations index 的依赖顺序，等待云端重新验证。
