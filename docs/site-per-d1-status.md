...
| P0 完整 Payload、后台、上传、内部缓存审计 | `6a94323` 已部署 REST/后台/R2/队列和公开 React 缓存/流式响应检查通过；[云端证据](site-per-d1-p0-deployed-validation.json) | 功能检查通过 |
| P0 生产 CPU、D1 读写、供应商用量、冷启动/内存 | [压缩后的指标](site-per-d1-p0-resource-metrics.json)：启动 42 ms，194 次请求无执行错误，内存 P95 137,775,000 bytes，仍超过文档限制；供应商账单未对账 | 未通过 |
...
- 实现已推送到 `feat/site-per-d1`。提交 `c7cf614` 的原型构建成功。`f39f791` 构建及 14 项浏览器检查通过，随后新库迁移因 CLI 文件排序先 ALTER 后 CREATE 失败，未部署 Worker；已改为显式使用仓库 migrations index 的依赖顺序。`f7c53e9` 的 446 项测试、完整构建与 14 项浏览器检查通过，两个测试库迁移完成，P0 Worker 已部署；线上登录返回 500，实时日志定位到全局对象缺少 beforeOperation 数组，正在修复。完整 P0 尚未通过。
...
- 最新 `6a94323` 构建 `b51405fe-04c1-4a0c-95e4-4801b794877d` 全链路成功。代码压缩将相同诊断的请求前 JS 堆从 102,690,040 降至 77,450,288 bytes；线上内存 P95 仍为 137,775,000 bytes，P99 为 138,547,550 bytes。下一步需要继续减少站点运行时的模块/配置加载占用；不得将诊断改善或零执行错误视为 P0 放行。生产仍为原部署，P1–P5 未启动。
