...
| P0 生产 CPU、D1 读写、供应商用量、冷启动/内存 | [24 小时原始指标](site-per-d1-baseline.json)已保存；冷启动、供应商额度及账单对账仍缺证据 | 实施中 |
...
- 已创建 [两个远程测试 D1](site-per-d1-p0-resources.json)，关闭读副本，目前尚未初始化 schema 或绑定已部署测试 Worker。云端 fixture 使用自己的本地隔离 D1；不能混淆这两组资源。
- 实现已推送到 `feat/site-per-d1`。最后一次实现提交 `c7cf614` 的构建已成功；后续提交仅增加文档证据，不需要重复构建。
