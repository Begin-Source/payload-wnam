...
目标始终为 [完整执行方案](site-per-d1-execution-plan.md)，以下是执行状态，不替代验收要求。2026-09-16T20:12Z，P0 原型可行性门槛通过；2026-09-18T10:38:52Z，P1 自动建站派发闭环通过。P1 其余交付仍在实施，P2–P5 尚待实施，不代表生产迁移、容量或完整计划验收通过。
...
提交 `d0bf5d8` 的普通构建 `c8f7705f-4095-4f66-85c3-e44213fbae0c` 成功完成 137 个测试文件、734 项测试、六站远程 smoke 和各 81 表只读核验，并在最后一步恢复经审核的第 3 次 Hook 失败。第 4 次 Hook 构建 `4d32e84d-2321-4af0-94c2-a48bb6810152` 随后独立运行成功，精确领取 F 请求、只读重入已完成的 checkpoint 6 操作并复用已验收分组 release。中央 D1 于 `2026-09-18T10:38:52.386Z` 将 attempt 4 记为 `succeeded/success`；原 D1 UUID、完成时间和六步回执不变。[完整记录](site-per-d1-p1-dispatch.md)。
...
中央 D1 已兼容升级到 v9，摘要 `b4e9f8f1805b1494a9bb33a8333036e7278c4491c5f1cfab0a4ad927ee2b8fdc`。直接查询确认投递表、两个索引和两个不可变保护触发器存在，当前投递数为 0。[实现边界与后续链路](site-per-d1-p1-data-delivery-journal.md)和[机器可读验证记录](site-per-d1-p1-data-delivery-validation.json)明确区分已部署的日志/RPC 基础与尚未接通的真实 Queue 生产者、站点消费者、候选应用和管理界面。本项和完整 P1 保持未通过。

## P1 历史接入：中央建站申请

独立验证分支 `feat/site-provision-admission` 已实现[持久申请与建站日志交接](site-per-d1-provision-admission.md)，并在 `4568591` 补齐原子数字 ID 分配、动态规划与已完成申请的完整分组历史。[构建 `f2ea9f46-cb15-47ef-bd2d-616553c21703`](site-per-d1-provision-planning-validation.json)于 `2026-09-18T00:43:28.390Z` 成功结束：672 项测试 / 130 个文件、14 项既有浏览器检查、完整角色构建与浏览器回归均通过，GitHub 检查成功。申请层 7 项、分组层 11 项原生 D1 测试覆盖编号竞争、连续新增、历史保持、同组排他及原计划恢复；外部部署检查和资源回执仍为受控测试替身。

中央 v5 迁移已在 `666b7c1` 接入正式角色 schema 和云端 bootstrap。新增七项完整 schema 测试通过 v1–v4 增量升级、已有数据/回执保留、回滚、响应丢失恢复、新库初始化及漂移拒绝；目标 273 对象。构建 `d1f4c9fc-83ca-416b-9a2c-3eb4e3b9327f` 于 `2026-09-18T01:12:05.702Z` 成功结束，679 项测试 / 130 个文件及完整应用回归通过，GitHub 检查成功。[迁移验证证据](site-per-d1-provision-migration-validation.json)保留首轮测试问题与修复范围。

申请表单、租户/负责人选项、按租户分页、单份摘要和取消 HTTP 现已挂在验证分支的中央角色。`6abeabe` 的[构建 `88fc3f39-7f92-4d10-8ac8-9a7c9041641f`](site-per-d1-admission-ui-validation.json)于 `2026-09-18T01:39:05.186Z` 成功结束，682 项测试 / 130 个文件、14 项既有浏览器检查和完整应用回归通过。新增中央浏览器检查覆盖表单、无效时区修正、提交与取消的响应丢失重试、唯一申请、无资源创建、手机布局及实时撤权；独立界面复核在该组件范围内为 ship。

`df7610c` 接通云端 `site:plan`（只读预览 / 持久准备）、构建产物 schema 核对及真实 Cloudflare 资源检查适配器。检查覆盖完整分组的最新 D1、实际绑定/域名/资源归属和前后部署一致性；重入保留原计划。构建 `d17f4928-61b6-42f9-907a-0d30e1b1e766` 于 `2026-09-18T02:02:39.151Z` 成功结束，696 项测试 / 131 个文件、14 项既有浏览器检查和完整应用回归通过，GitHub 检查成功。[云端规划证据](site-per-d1-cloud-planning-validation.json)记录 12 项受控 API 检查与 12 项原生分组测试；新命令尚未在远程 v5 上执行。

`62a9106` 的[动态普通发布验证](site-per-d1-dynamic-release-validation.json)于 `2026-09-18T02:33:07.043Z` 成功结束：711 项测试 / 132 个文件、14 项既有浏览器检查及完整角色/主应用回归通过，GitHub 检查成功。发布控制器、有效清单、全部成员运行证明及完整核验目标均改为读取持久历史，移除普通发布前后的固定 D 重入。原生测试证明新增两站后的两组上传中断恢复不重复上传，原回执保留；plain Node 消费者与控制器复用同一历史校验器。运行证明支持多名员工身份投影及暂停状态，整套 P1 浏览器 smoke 仍限合成员工 7 和可进入的测试站点。

上述验证分支没有部署；02:33:56Z 的直接核对仍为中央 v4。随后 `86c9306` 移除旧的仅恢复选择，合入 `feat/site-per-d1`，通过下述真实测试资源发布。中央 v5、申请 HTTP/界面和动态普通发布控制器现已部署。申请执行、构建派发及结果对账仍待接通，界面继续明确提示自动执行尚未启用，不能宣称自动建站已可用。
...
`86c9306` / 构建 `9abce112-0fe4-44ec-b111-0a15043229aa` 于 `2026-09-18T03:09:53.160Z` 成功结束，GitHub 检查 completed/success：711 项测试 / 132 个文件、14 项既有浏览器检查、完整角色及主应用检查、P0 实际回归、P1 四站真实 HTTPS 后台验收全部通过。[发布与直接核验证据](site-per-d1-admission-release-validation.json)。全部安装、测试、构建和部署由 Cloudflare Builds 执行；本地仅源码、文档、Git、静态检查和证据摘要比对。

中央从 v4 原子增量升级为 v5，新增七个对象，总计 273 个；实际 schema 摘要与回执一致。中央 deployment `5e2eafa3-b8b6-49f2-b7bb-1e716f6f09bb`，站点 deployment `c4f8bec4-1c5e-45bb-9815-bc1ab7122c6a`，运行提交均为本次源码。动态控制器按完整 C/D 历史发布 A/B/C/D；普通发布完成于 03:09:03Z，重复执行不上传、回执不变。

真实后台提交、同 UUID 重试、唯一持久申请、私有摘要、取消确认/重试和桌面/手机检查通过。合成申请 `fc113d09-0c60-4b09-8083-8948184a2def` 分配数字 ID 105，已取消，prepared 为空，无建站操作或新 D1。取消仍保留编号。四站编辑、同 ID 并发隔离、撤权、暂停/恢复、MCP 和退出均通过，浏览器错误为零。[324 表逐表报告](site-per-d1-admission-release-site-report.json)从 36 段云端记录重建，完整报告与各站摘要均匹配。

03:10:31Z 直接查询确认无未完成发布、组租约归零、C/D 原六步回执摘要不变；四站 active、版本 83/1/2/2、生产关闭、经理权限恢复。生产 deployment 保持 `3046ffb1-b8ad-47ba-a373-9be5d0526c4b`。云端代理有临时连接中断日志，但完整验收与平台终态均成功。此次没有执行动态申请建站或启用自动派发，远程仍为一个四站组；完整 P1 和 P2–P5 未完成。

## P1 前轮发布：多请求历史清单与逐组发布协调
...
该恢复轮的 `operations/p1-release.json` 曾显式固定仅恢复选择；`86c9306` 已移除，并完成新的普通功能发布。中央建站入口、多组管理、主数据界面/队列、完整数据图与混合版本核验、其余 P1 和 P2–P5 继续实施，完整目标未完成。
