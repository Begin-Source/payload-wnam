...
该分支按现有触发器只验证，不部署。01:12Z 直接核对中央七个申请 schema 对象均不存在，远程仍为 v4，生产和 P1 部署未变化。远程迁移、HTTP、Payload 界面、真实 Cloudflare 规划检查、构建触发和动态执行入口尚待接通；不能据此宣称中央建站已可用。当前已部署版本仍见下方发布记录。
