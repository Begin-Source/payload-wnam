...
独立验证分支 `feat/site-provision-admission` 的 `dbc4642` 已实现[持久申请与建站日志交接](site-per-d1-provision-admission.md)：严格人员输入、租户权限、重复提交、不可变准备计划，以及取消和预约的原子竞争。[构建 `d8869001-a0c0-441e-bad8-c484c6b8965c`](site-per-d1-provision-admission-validation.json)于 `2026-09-18T00:17:48.159Z` 成功结束；新增 6 项原生 D1 测试、总计 667 项测试 / 130 个文件、14 项既有浏览器检查、完整角色构建与浏览器回归均通过，GitHub 检查成功。

该分支按现有触发器只验证，不部署。申请 schema 未安装到远程数据库，HTTP、Payload 界面、数字 ID 分配、动态资源规划和云端执行入口尚待接通；不能据此宣称中央建站已可用。当前已部署版本仍见下方发布记录。
