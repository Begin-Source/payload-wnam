...
export default {
  async fetch(request: Request, env: Env) {
    ...
    return withSiteContext(context(env, siteId), async () => {
      ...
      const uploaded = await media.put(mediaKey, siteId,{ httpMetadata: { contentType: 'text/plain',cacheControl: 'public, max-age=300' } })
      assert(uploaded?.key === mediaKey, 'R2 relative key mismatch')
      const object = await media.get(mediaKey)
      const mediaHeaders = new Headers()
      object!.writeHttpMetadata(mediaHeaders)
      assert(mediaHeaders.get('content-type') === 'text/plain' && mediaHeaders.get('cache-control') === 'public, max-age=300', 'R2 response metadata mismatch')
      assert(await object?.text() === siteId, 'Cross-site media')
