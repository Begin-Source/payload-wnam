...
export function p1Manifests() {
  ...
  const expected = [
    ...
    { config: site,name: 'payload-wnam-p1-sites',hosts: ['cms-site-p1-a.beginos.org','cms-site-p1-b.beginos.org'],
      dbs: [['SITE_D1_A','payload-wnam-p1-a','6daecf80-30f5-4010-add6-d67bb556cc42'],['SITE_D1_B','payload-wnam-p1-b','b2a1a340-681a-4581-8817-b2f9629a6ed4']],
  ...
  assert.deepEqual(JSON.parse(site.vars.SITE_ROUTES),site.d1_databases.map((db,index) => ({
    siteId: index ? 'p1-b' : 'p1-a',localSiteId: index ? 82 : 37,bindingName: db.binding,databaseId: db.database_id,schemaVersion: 1,
  })))
  return { central,site }
}
