...
assert.equal(domains.find(domain => domain.hostname === 'agenthub.beginos.org')?.service,'payload-wnam')
...
assert.deepEqual(await query('SELECT role,digest,operation_id,completed FROM p1_schema_bootstrap WHERE id=1'),
  [{ role: 'central',digest: schemaDigest,operation_id: 'p1-central-schema-v9',completed: 1 }])
