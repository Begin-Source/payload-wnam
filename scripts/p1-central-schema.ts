...
export const P1_CENTRAL_V8 = 'a204fb805598726ccd3f5629b390a0896352b748edc6cd3a42d04bfcbf5400db'

/** Upgrade only the previously reviewed complete schemas, never silently adopt
 * another schema. Fresh installs use v9; old versions pass through each prior additive
...
export async function applyP1CentralSchema(database: D1Database,schema: RoleSchema) {
  ...
  if (dataDeliverySchemaObjects.some(name => schema.objects.filter(item => item.name === name).length !== 1)) {
  ...
  if (provisionDispatchRunSchemaObjects.some(name => v8.objects.filter(item => item.name === name).length !== 1)) {
    throw new Error('Incomplete central dispatch run schema')
  }
  const v6: RoleSchema = { ...v8,objects: v8.objects.filter(item => !runObjects.has(item.name)) }
  ...
  if (!table) return applyP1Schema(database,schema,'p1-central-schema-v9')
  if (table) {
    ...
    if (current?.digest === targetDigest && current.operation_id === 'p1-central-schema-v9') {
      return applyP1Schema(database,schema,'p1-central-schema-v9')
    }
  ...
  let current = await database.prepare('SELECT digest,operation_id,completed FROM p1_schema_bootstrap WHERE id=1')
    .first<{ digest: string; operation_id: string; completed: number }>()
  if (current?.digest !== P1_CENTRAL_V8 || current.operation_id !== 'p1-central-schema-v8') {
    ...
    const attempt = v8.objects.find(item => item.name === 'site_provision_dispatch_attempts')
    const immutable = v8.objects.find(item => item.name === 'site_provision_dispatch_run_immutable')
    ...
    current = await database.prepare('SELECT digest,operation_id,completed FROM p1_schema_bootstrap WHERE id=1')
      .first<{ digest: string; operation_id: string; completed: number }>()
    if (current?.digest === v7Digest && current.operation_id === 'p1-central-schema-v7' && current.completed === 1) {
      await database.batch([
        ...
        database.prepare(`INSERT INTO site_control_schema_migrations (operation_id,from_digest,to_digest,applied_at) VALUES (?,?,?,?)`)
          .bind('p1-central-schema-v8',v7Digest,P1_CENTRAL_V8,appliedAt),
        database.prepare(`UPDATE p1_schema_bootstrap SET digest=?,operation_id=?,completed_at=?
          ...
          .bind(P1_CENTRAL_V8,'p1-central-schema-v8',appliedAt,v7Digest),
      ])
    }
  }
  return applyP1Schema(database,schema,'p1-central-schema-v9',{
    fromDigest: P1_CENTRAL_V8,fromOperationId: 'p1-central-schema-v8',addedObjects: dataDeliverySchemaObjects,
  })
}
