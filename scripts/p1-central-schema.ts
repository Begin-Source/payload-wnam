 ...
 * another schema. Fresh installs use v5; old versions pass through each additive
...
export async function applyP1CentralSchema(database: D1Database,schema: RoleSchema) {
  ...
  if (provisionAdmissionSchemaObjects.some(name => schema.objects.filter(item => item.name === name).length !== 1)) {
    throw new Error('Incomplete central admission schema')
  }
  const v4: RoleSchema = { ...schema,objects: schema.objects.filter(item => !admissionObjects.has(item.name)) }
  ...
  return applyP1Schema(database,schema,'p1-central-schema-v5',{
