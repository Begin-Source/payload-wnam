...
export async function checkLifecycleBrowser({ hub,siteId,otherSiteId,siteRequest,otherSiteRequest,artifactPrefix }) {
  ...
  try {
    ...
    assert.equal(JSON.parse(replay.body).routingVersion,initial.routingVersion+1)
    ...
    const ambiguousReceipt = await write(ambiguousOperation)
    assert.equal(ambiguousReceipt.status,200)
    assert.equal(JSON.parse(ambiguousReceipt.body).replayed,true)
